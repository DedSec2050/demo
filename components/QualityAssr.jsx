...
const QualityAssr = () => {
  return (
    <div className="bg-grey-600 flex w-full flex-col items-center justify-center">
      <h1 className="mb-[5vh]">
        <span className="bg-gradient-to-r from-accent via-grn to-accent bg-clip-text text-center text-clampheader font-bold text-transparent xxl:text-[2.5vw]">
          Quality Assurance at Cybernous
        </span>
      </h1>
      <p className="w-[80vw] text-left text-clamptextmd font-semibold xxl:mt-[6vh] xxl:text-[1.5vw]">
      ...
      {listdata.map((data, index) => {
        return (
          <li
            key={index}
            className="w-[80vw] list-none text-clamptextmd font-semibold xxl:text-[1.3vw] xxl:leading-[55px]"
          >
            {index + 1}. {data.data}
          </li>
        );
